...
year, mon, day = 2020, 7, 14
_year, _mon, _day = str(year), str(mon), str(day)
beg_time = dt.datetime(year, mon, day, 11, 30, 0, 000000)
end_time = dt.datetime(year, mon, day, 12, 0, 0, 000000)
...
swa_datetime_list = cdflib.cdfepoch.encode(epoch)
swa_datetime = [datetime.datetime.strptime(t[0:-3],"%Y-%m-%dT%H:%M:%S.%f") for t in swa_datetime_list]

start_ind = bisect.bisect_left(swa_datetime, beg_time)
stop_ind = bisect.bisect_right(swa_datetime, end_time)
epoch_lst = swa_datetime[start_ind:stop_ind]
energy = data.varget('Energy')  #'UNITS': 'eV'
elevation = data.varget('Elevation')
azimuth = data.varget('Azimuth')
...
pas_to_rtn_lst = rotmat_rtn_inst[start_ind:stop_ind, :, :]
vdf_lst = vdf[start_ind:stop_ind, :, :, :]
# read magnetic field cdf file
mag_l2_dir = r'E:\程序\Python\SolVDF\data\solar_orbiter_data\mag\science\l2\rtn-normal'
mag_name = 'solo_l2_mag-rtn-normal_20200714_v03.cdf'
mag_file = os.path.join(mag_l2_dir, mag_name)
magdata = cdflib.CDF(mag_file)
mag_epoch = magdata.varget('Epoch')
mag_datetime_list = cdflib.cdfepoch.encode(mag_epoch)
mag_datetime = [datetime.datetime.strptime(t[0:-3],"%Y-%m-%dT%H:%M:%S.%f") for t in mag_datetime_list]
mag_rtn = magdata.varget('B_RTN')

# mag_file = 'solo_l2_mag-rtn-normal_20200714_v03.cdf'
# mag_cdf = pycdf.CDF(dir_cdf + mag_file)
# print(mag_cdf)

start_ind = bisect.bisect_left(mag_datetime, beg_time)
stop_ind = bisect.bisect_right(mag_datetime, end_time)
mag_epoch = mag_datetime[start_ind:stop_ind]
br_lst = mag_rtn[start_ind:stop_ind, 0]
bt_lst = mag_rtn[start_ind:stop_ind, 1]
bn_lst = mag_rtn[start_ind:stop_ind, 2]
# interpolation
...
for i_time, epoch in enumerate(epoch_lst):
    ...
    vpara_core_vel = vpara_rtn[np.unravel_index(max_ind, (11, 9, 96))]
    vperp1_core_vel = vperp1_rtn[np.unravel_index(max_ind, (11, 9, 96))]
    vperp2_core_vel = vperp2_rtn[np.unravel_index(max_ind, (11, 9, 96))]
    vpara = vpara_rtn - vpara_core_vel
    vperp1 = vperp1_rtn - vperp1_core_vel
    vperp2 = vperp2_rtn - vperp2_core_vel

    # save the central core velocity
    central_core_vel = [vpara_core_vel, vperp1_core_vel, vperp2_core_vel]
    dir_solution = 'output/central_core_velocity/'
    filename = 'proton_core_velocity_' + epoch.strftime('%H%M%S.%f')[:-3] + '.txt'
    np.savetxt(dir_solution + filename, central_core_vel, fmt='%.10f')

    # save magnetic field direction in the RTN coordinates
    dir_solution = 'output/magnetic_field_direction/'
    filename = 'magnetic_field_direction_' + epoch.strftime('%H%M%S.%f')[:-3] + '.txt'
    np.savetxt(dir_solution + filename, np.c_[epara, eperp1, eperp2], fmt='%.10f')

    # save pas to rtn matrix
    dir_solution = 'output/pas_to_rtn/'
    filename = 'pas_to_rtn_' + epoch.strftime('%H%M%S.%f')[:-3] + '.txt'
    np.savetxt(dir_solution + filename, pas_to_rtn, fmt='%.10f')
    ...
    # ===================================================
    if is_plot_original_vdf == 1:
        dir_fig = 'figure/regular_int_vdf/'
    ...
    # ===================================================
        ...
        dir_fig = 'figure/compare_obs_fit_vdf/'
